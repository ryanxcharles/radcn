...
test('candidate switch exposes checkbox role switch hooks and forms', async ({ page }) => {
  await page.goto(`${candidate}/fixtures/switch/demo`)
  let demo = page.locator('[data-radcn-fixture-switch-family="switch-demo"]')
  let demoInput = demo.locator('input[data-radcn-switch-input]')
  let demoWrapper = demo.locator('[data-radcn-switch-wrapper]')
  let demoLabel = demo.locator('[data-radcn-label]')
  await expect(demo).toBeVisible()
  await expect(demo).toHaveClass(/flex/)
  await expect(demo).toHaveClass(/items-center/)
  await expect(demo).toHaveClass(/space-x-2/)
  await expect(demo).toHaveCSS('display', 'flex')
  await expect(demo).toHaveCSS('align-items', 'center')
  await expect(demo).toHaveCSS('gap', '8px')
  await expect(demoInput).toHaveAttribute('id', 'airplane-mode')
  await expect(demoInput).toHaveAttribute('role', 'switch')
  await expect(demoInput).toHaveAttribute('type', 'checkbox')
  await expect(demoInput).toHaveAccessibleName('Airplane Mode')
  await expect(demoInput).not.toBeChecked()
  await expect(demoWrapper).toHaveAttribute('data-state', 'unchecked')
  await expect(demoWrapper).toHaveAttribute('data-size', 'default')
  await expect(demoInput).toHaveAttribute('data-state', 'unchecked')
  await expect(demoInput).toHaveAttribute('data-size', 'default')
  await expect(demo.locator('[data-radcn-switch-thumb]')).toHaveCount(1)
  await expect(demoLabel).toHaveAttribute('for', 'airplane-mode')
  await expect(demoLabel).toHaveText('Airplane Mode')
  await demoLabel.click()
  await expect(demoInput).toBeChecked()
  await expect(demoWrapper).toHaveAttribute('data-state', 'checked')
  await expect(demoInput).toHaveAttribute('data-state', 'checked')
